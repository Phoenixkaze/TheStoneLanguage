...
public class Parser {
    ...
    protected static class Repeat extends Element {
                ...
                throws ParseException
        {
            while (parser.match(lexer)) {
                ...
                if (t.getClass() != AbstractSyntaxTreeList.class || t.numberOfChildren() > 0)
                    res.add(t);
                if (onlyOnce)
                    break;
            }
    ...
    protected static abstract class AToken extends Element {
        ...
        protected AToken(Class<? extends AbstractSyntaxTreeLeaf> type) {
            if (type == null)
                type = AbstractSyntaxTreeLeaf.class;
    ...
    protected static class IdToken extends AToken {
        ...
        protected IdToken(Class<? extends AbstractSyntaxTreeLeaf> type, HashSet<String> r) {
            super(type);
    ...
    protected static class NumToken extends AToken {
        protected NumToken(Class<? extends AbstractSyntaxTreeLeaf> type) { super(type); }
        protected boolean test(Token t) { return t.isNumber(); }
    }

    protected static class StrToken extends AToken {
        protected StrToken(Class<? extends AbstractSyntaxTreeLeaf> type) { super(type); }
    ...
    protected static class Leaf extends Element {
        ...
        protected void find(List<AbstractSyntaxTree> res, Token t) {
            res.add(new AbstractSyntaxTreeLeaf(t));
        }
        protected boolean match(Lexer lexer) throws ParseException {
    ...
    protected static class Expr extends Element {
        ...
        protected Expr(Class<? extends AbstractSyntaxTree> clazz, Parser exp,
                       Operators map)
        {
            factory = Factory.getForAbstractSyntaxTreeList(clazz);
            ops = map;
            factor = exp;
        }
                ...
                throws ParseException
        {
            ...
            list.add(left);
            list.add(new AbstractSyntaxTreeLeaf(lexer.read()));
            AbstractSyntaxTree right = factor.parse(lexer);
            Precedence next;
    ...
    protected static abstract class Factory {
        ...
        protected static Factory getForAbstractSyntaxTreeList(Class<? extends AbstractSyntaxTree> clazz) {
                ...
                f = new Factory() {
                    protected AbstractSyntaxTree make0(Object arg) throws Exception {
                        ...
                        if (results.size() == 1)
                            ...
                            return new AbstractSyntaxTreeList(results);
                    }
                };
            return f;
        }
    ...
    public Parser number(Class<? extends AbstractSyntaxTreeLeaf> clazz) {
    ...
    public Parser identifier(Class<? extends AbstractSyntaxTreeLeaf> clazz,
                             HashSet<String> reserved)
    {
    ...
    public Parser string(Class<? extends AbstractSyntaxTreeLeaf> clazz) {
